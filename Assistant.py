...
def load_commands(file_path):
    with open("C://Users//nanyh//PycharmProjects//pythonProject//commands.json") as file:
        data = json.load(file)
...
def wishMe():
    ...
    if 0 <= hour < 12:
        ...
        speak("Good Evening")
    speak("HI I am Andrew,  How can I assist you?")
...
def get_weather(city):
    ...
    url = f"https://api.openweathermap.org/data/2.5/weather?q={city}&appid={api_key}&units"
    ...
    if data["cod"] != "404":
        ...
        message = f"The weather in {city} is {weather}, with a temperature of {temperature}"
        return message
    else:
    ...
    if matched_command:
        if matched_command == "greeting":
            ...
            if "stop" in user_preference.lower():
                speak("Alright, let's end the conversation here.")
                return
            speak(f"I enjoy {user_preference} too. There's something special about holding")
